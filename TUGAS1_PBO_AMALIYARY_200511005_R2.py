...
def insert_data(conn):
   ...
   sql = "INSERT INTO mahasiswa2 (idmhs, nim, nama, idfakultas, idprodi) VALUES (%s, %s, %s, %s, %s)"
   cur = conn.cursor()
   cur.execute(sql, val)
   conn.commit()
   print("==================================")
...
def update_data(conn):
   ...
   sql = "UPDATE mahasiswa2 SET nim=%s, nama=%s, idfakultas=%s, idprodi=%s WHERE idmhs=%s"
...
def delete_data(conn):
   ...
   slc = "SELECT * FROM mahasiswa2 WHERE idmhs= %s"
   val = (idmhs)
   cur.execute(slc, val)
   con = cur.rowcount
   if (con == 1):
      ...
      if (inp.upper()=="Y"):
         sql = "DELETE FROM mahasiswa2 WHERE idmhs=%s"
         val = (idmhs)
         cur.execute(sql, val)
         conn.commit()
         print("==================================")
...
def search_data(conn):
   ...
   sql = "SELECT * FROM mahasiswa2 WHERE nim LIKE %s OR nama LIKE %s OR nama LIKE %s OR nama LIKE %s"
...
def show_menu(conn):
   ...
   print("== DIBUAT OLEH AMALIYAH RISKIYANI ==")
   print("1. Insert Data")
   print("2. Show Data")
   print("3. Update Data")  
   print("4. Delete Data")
